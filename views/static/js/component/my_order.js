let constructMyOrder =  {
	...
	constructMyOrder: function(page) {
		let html = '';
		...
		for (let i = page * this.idPerPage; i < Math.min((page + 1) * this.idPerPage, this.allData.length); ++i) {
			...
			let sendPersonNm = this.allData[i].sendPersonNm;
			let sendTelno = this.allData[i].sendTelno;
			let receivePersonNm = this.allData[i].receivePersonNm;
			let receiveTelno = this.allData[i].receiveTelno;
			let content = '';
			...
			html += '</td>';
			html += '<td>';
			html += '<div id="send_person_nm" class="send-person-nm">' + sendPersonNm + '</div>';
			html += '</td>';
			html += '<td>';
			html += '<div id="send_telno" class="send-telno">' + sendTelno + '</div>';
			html += '</td>';
			html += '<td>';
			html += '<div id="receive_person_nm" class="receive-person-nm">' + receivePersonNm + '</div>';
			html += '</td>';
			html += '<td>';
	...
	constructPagination: function() {
		...
		$('.page-no').click(function () {
			this.constructMyOrder(Number($(this).text()) - 1);
		});
		$('#prev_page').click(function () {
			...
			if (this.cur > 0) {
				...
				this.constructMyOrder(cur * this.pageLength);
			}
		});
	...
	selectCallback: function(data) {
		...
		this.cur += 1;
		this.constructPagination();
		this.constructMyOrder(this.cur * this.pageLength);
